...
Connects *wellConnected(Graph g, int *nconns)
{
   ...
   *nconns = 0;
   ...
   for (i = 0; i < g->nV; i++) {
      connection = 0;
      for (j = 0; j < g->nV; j++) {
         if (g->edges[i][j]) connection++;
      }
      if (connection >= 2) {
         for (k = 0; k < (*nconns); k++) {
            // if the connection more than the next
            if (connection > toReturn[k].nconn) {
               // shift everything backward
               for (l = (*nconns)-1; l >= k; l--) {
                  toReturn[l+1].vertx = toReturn[l].vertx;
                  toReturn[l+1].nconn = toReturn[l].nconn;
               }
               break;
            }
         }
         toReturn[k].vertx = i;
         toReturn[k].nconn = connection;
         (*nconns)++;
      }
   }
   return toReturn;
